...
**Looply** provides a transparent layer that organizes and simplifies the information you need, when you need it. The goal is to deliver clear project updates and summaries to different parts of the organization without requiring people to jump between tools.

## Core Concepts

Looply’s data model is structured around a few key entities:
...
- **ImportJobs**: Represent each request to bring external data into Looply.
...
Looply accepts external data through a queued import flow. This avoids blocking callers and allows heavy normalization to run asynchronously.
...
By unifying updates from multiple tools into entries and entry_updates, Looply makes it easier to see progress, discussions, and context in one place. This enables project summaries and updates to be shared across the organization without requiring deep knowledge of each individual system.
