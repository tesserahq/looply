...
def test_add_contact_to_list(db, test_contact_list, test_contact):
    ...
    repository = ContactListRepository(db)

    member = repository.add_contact_to_list(test_contact_list.id, test_contact.id)
...
def test_add_contact_to_list_already_exists(db, test_contact_list, test_contact):
    ...
    # Add once
    member1 = repository.add_contact_to_list(test_contact_list.id, test_contact.id)
    assert member1 is not None

    # Try to add again
    member2 = repository.add_contact_to_list(test_contact_list.id, test_contact.id)
...
def test_add_contact_to_nonexistent_list(db, test_contact):
    ...
    member = repository.add_contact_to_list(nonexistent_list_id, test_contact.id)
...
def test_add_nonexistent_contact_to_list(db, test_contact_list):
    ...
    member = repository.add_contact_to_list(
...
def test_remove_contact_from_list(db, test_contact_list, test_contact):
    ...
    repository.add_contact_to_list(test_contact_list.id, test_contact.id)

    # Remove it
    success = repository.remove_contact_from_list(test_contact_list.id, test_contact.id)
    assert success is True

    # Verify it's removed (check member count is 0)
    count = repository.get_list_member_count(test_contact_list.id)
    assert count == 0
...
def test_remove_contact_from_list_not_found(db, test_contact_list):
    ...
    success = repository.remove_contact_from_list(
        test_contact_list.id, nonexistent_contact_id
    )
...
def test_get_list_members(db, test_contact_list, test_contact, faker, test_user):
    ...
    repository.add_contact_to_list(test_contact_list.id, test_contact.id)
    repository.add_contact_to_list(test_contact_list.id, another_contact.id)

    # Get members
    members = repository.get_list_members(test_contact_list.id)
...
def test_is_contact_in_list(db, test_contact_list, test_contact):
    ...
    assert repository.is_contact_in_list(test_contact_list.id, test_contact.id) is False

    # Add to list
    repository.add_contact_to_list(test_contact_list.id, test_contact.id)

    # Now should be in list
    assert repository.is_contact_in_list(test_contact_list.id, test_contact.id) is True
...
def test_is_contact_in_list_after_removal(db, test_contact_list, test_contact):
    ...
    # Add and then remove
    repository.add_contact_to_list(test_contact_list.id, test_contact.id)
    repository.remove_contact_from_list(test_contact_list.id, test_contact.id)
...
def test_get_contact_lists_for_contact(
    ...
    repository.add_contact_to_list(test_contact_list.id, test_contact.id)
    repository.add_contact_to_list(another_list.id, test_contact.id)

    # Get lists for contact
    lists = repository.get_contact_lists_for_contact(test_contact.id)
...
def test_add_multiple_contacts_to_list(
    ...
    # Add all contacts
    added_count = repository.add_contacts_to_list(
...
def test_remove_multiple_contacts_from_list(
    ...
    repository.add_contact_to_list(test_contact_list.id, test_contact.id)
    repository.add_contact_to_list(test_contact_list.id, contact2.id)

    # Remove all
    removed_count = repository.remove_contacts_from_list(
...
def test_clear_list_members(db, test_contact_list, test_contact, faker, test_user):
    ...
    repository.add_contact_to_list(test_contact_list.id, test_contact.id)
    repository.add_contact_to_list(test_contact_list.id, contact2.id)
    repository.add_contact_to_list(test_contact_list.id, contact3.id)

    # Clear all
    removed_count = repository.clear_list_members(test_contact_list.id)
