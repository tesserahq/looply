...
def test_create_contact(db, sample_contact_data):
    ...
    contact = ContactRepository(db).create_contact(contact_create)
...
def test_get_contact(db, test_contact):
    ...
    retrieved_contact = ContactRepository(db).get_contact(test_contact.id)
...
def test_get_contact_by_email(db, test_contact):
    ...
    retrieved_contact = ContactRepository(db).get_contact_by_email(test_contact.email)
...
def test_get_contact_by_phone(db, test_contact):
    ...
    retrieved_contact = ContactRepository(db).get_contact_by_phone(test_contact.phone)
...
def test_get_contacts(db, test_contact):
    ...
    contacts = ContactRepository(db).get_contacts()
...
def test_get_contacts_by_creator(db, test_contact):
    ...
    contacts = ContactRepository(db).get_contacts_by_creator(test_contact.created_by_id)
...
def test_get_active_contacts(db, test_contact, inactive_contact):
    ...
    active_contacts = ContactRepository(db).get_active_contacts()
...
def test_update_contact(db, test_contact):
    ...
    # Update contact
    updated_contact = ContactRepository(db).update_contact(
        test_contact.id, contact_update
    )
...
def test_delete_contact(db, test_contact):
    ...
    # Delete contact
    success = contact_repository.delete_contact(test_contact.id)

    # Assertions
    assert success is True
    deleted_contact = contact_repository.get_contact(test_contact.id)
...
def test_contact_not_found_cases(db):
    ...
    assert contact_repository.get_contact(non_existent_id) is None

    # Get by non-existent email
    assert contact_repository.get_contact_by_email("nonexistent@example.com") is None

    # Get by non-existent phone
    assert contact_repository.get_contact_by_phone("+1234567890") is None

    # Update non-existent contact
    update_data = {"first_name": "Updated"}
    contact_update = ContactUpdate(**update_data)
    assert contact_repository.update_contact(non_existent_id, contact_update) is None

    # Delete non-existent contact
    assert contact_repository.delete_contact(non_existent_id) is False
...
def test_search_contacts_with_filters(db, test_contact):
    ...
    results = ContactRepository(db).search(filters)
...
def test_restore_contact(db, test_contact):
    ...
    contact_repository.delete_contact(test_contact.id)
    assert contact_repository.get_contact(test_contact.id) is None

    # Then restore it
    success = contact_repository.restore_contact(test_contact.id)
    assert success is True

    # Verify it's restored
    restored_contact = contact_repository.get_contact(test_contact.id)
...
def test_hard_delete_contact(db, test_contact):
    ...
    success = contact_repository.hard_delete_contact(test_contact.id)
    assert success is True
...
def test_get_deleted_contacts(db, test_contact):
    ...
    deleted_contacts = contact_repository.get_deleted_contacts()
...
def test_get_deleted_contact(db, test_contact):
    ...
    deleted_contact = contact_repository.get_deleted_contact(test_contact.id)
...
def test_get_contacts_deleted_after(db, test_contact):
    ...
    deleted_contacts = contact_repository.get_contacts_deleted_after(past_date)
    assert len(deleted_contacts) >= 1
    assert any(c.id == test_contact.id for c in deleted_contacts)
...
def test_search_text(db, test_contact):
    ...
    results = contact_repository.search_text(test_contact.first_name)
    assert len(results) >= 1
    assert any(c.id == test_contact.id for c in results)

    # Search by last name
    results = contact_repository.search_text(test_contact.last_name)
    assert len(results) >= 1
    assert any(c.id == test_contact.id for c in results)

    # Search by company
    if test_contact.company:
        results = contact_repository.search_text(test_contact.company)
        assert len(results) >= 1
        assert any(c.id == test_contact.id for c in results)

    # Search by email
    if test_contact.email:
        results = contact_repository.search_text(test_contact.email)
        assert len(results) >= 1
        assert any(c.id == test_contact.id for c in results)

    # Search with no match
    results = contact_repository.search_text("nonexistentsearchterm12345")
    assert len(results) == 0
...
def test_search_text_with_pagination(db, test_contact):
    ...
    results = contact_repository.search_text(test_contact.first_name, skip=0, limit=10)
    assert isinstance(results, list)
...
def test_search_text_case_insensitive(db, test_contact):
    ...
    results_lower = contact_repository.search_text(test_contact.first_name.lower())

    # Search with uppercase
    results_upper = contact_repository.search_text(test_contact.first_name.upper())
